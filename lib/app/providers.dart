...
final cartCountProvider=FutureProvider<int>((ref)async{try{final cart=await ref.watch(cartRepositoryProvider).load();return cart.items.fold<int>(0,(sum,item)=>sum+item.quantity);}catch(_){return 0;}});
final engagementRepositoryProvider=Provider<EngagementRepository>((ref)=>EngagementRepository(ref.watch(apiClientProvider)));
final notificationsDataProvider=FutureProvider<Map<String,dynamic>>((ref)async{try{return await ref.watch(engagementRepositoryProvider).notifications();}catch(_){return const{'notifications':<dynamic>[]};}});
...
final storeReviewsProvider=FutureProvider.family<Map<String,dynamic>,String>((ref,id)=>ref.watch(engagementRepositoryProvider).storeReviews(id));
