...
class OffersScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<OffersScreen> createState() => _OffersScreenState();
}
...
class _OffersScreenState extends ConsumerState<OffersScreen> {
  ...
  List<ProductModel> _apply(List<ProductModel> all) {
    ...
    if (sort == 'priceAsc') x.sort((a, b) => a.price.compareTo(b.price));
    else if (sort == 'priceDesc') x.sort((a, b) => b.price.compareTo(a.price));
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => Padding(
          padding: EdgeInsets.fromLTRB(17, 17, 17, MediaQuery.of(ctx).viewInsets.bottom + 24),
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            const Text('فلترة وترتيب العروض', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(initialValue: sort, decoration: const InputDecoration(labelText: 'الترتيب'), items: const [DropdownMenuItem(value: 'discount', child: Text('أعلى خصم')), DropdownMenuItem(value: 'priceAsc', child: Text('السعر الأقل')), DropdownMenuItem(value: 'priceDesc', child: Text('السعر الأعلى')), DropdownMenuItem(value: 'rating', child: Text('الأعلى تقييماً'))], onChanged: (v) { if (v != null) setLocal(() => sort = v); }),
            const SizedBox(height: 10),
            Wrap(spacing: 7, children: [0, 10, 20, 30, 40].map((v) => ChoiceChip(label: Text(v == 0 ? 'كل العروض' : 'خصم $v% فأكثر'), selected: minDiscount == v, onSelected: (_) => setLocal(() => minDiscount = v))).toList()),
            const SizedBox(height: 10),
            Wrap(spacing: 7, children: <MapEntry<String, String>>[const MapEntry('all', 'كل الأسعار'), const MapEntry('under5', 'أقل من 5 دولار'), const MapEntry('5to25', '5 – 25 دولار'), const MapEntry('over25', 'أكثر من 25 دولار')].map((v) => ChoiceChip(label: Text(v.value), selected: price == v.key, onSelected: (_) => setLocal(() => price = v.key))).toList()),
            const SizedBox(height: 10),
            Wrap(spacing: 7, children: [0.0, 4.0, 4.5].map((v) => ChoiceChip(label: Text(v == 0 ? 'كل التقييمات' : '$v نجوم فأعلى'), selected: minRating == v, onSelected: (_) => setLocal(() => minRating = v))).toList()),
            const SizedBox(height: 14),
            FilledButton(style: FilledButton.styleFrom(backgroundColor: spikeRed), onPressed: () { setState(() {}); Navigator.pop(ctx); }, child: const Text('عرض النتائج')),
          ]),
        ),
      ),
    );
      ...
      child: state.when(
        ...
        data: (all) {
          ...
          return Column(children: [
            Padding(padding: const EdgeInsets.fromLTRB(17, 12, 17, 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('العروض والخصومات', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w800)), TextButton.icon(onPressed: _filters, icon: const Icon(Icons.tune, size: 18), label: const Text('فلترة وترتيب'))])),
            Expanded(
              child: list.isEmpty
