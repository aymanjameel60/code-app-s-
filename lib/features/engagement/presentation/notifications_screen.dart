...
Future<void> _load()async{if(mounted)setState(()=>loading=true);try{data=await ref.read(engagementRepositoryProvider).notifications();}catch(e){if(mounted)showSpikeToast(context,e.toString());}finally{if(mounted)setState(()=>loading=false);}}
@override Widget build(BuildContext context){if(loading)return const Scaffold(body:SpikeLoading());final items=(data?['notifications'] as List? ?? const []).whereType<Map>().map((e)=>Map<String,dynamic>.from(e)).toList();return Scaffold(appBar:AppBar(backgroundColor:Colors.transparent,title:const Text('الإشعارات'),centerTitle:true,actions:[if(items.isNotEmpty)TextButton(onPressed:()async{await ref.read(engagementRepositoryProvider).readAllNotifications();await _load();},child:const Text('قراءة الكل'))]),body:items.isEmpty?const SpikeEmptyState(message:'لا توجد إشعارات'):RefreshIndicator(onRefresh:_load,child:ListView.builder(padding:const EdgeInsets.all(17),itemCount:items.length,itemBuilder:(context,i){final n=items[i],unread=n['read_at']==null;return Container(margin:const EdgeInsets.only(bottom:9),decoration:BoxDecoration(color:unread?spikePanel:Colors.white,borderRadius:BorderRadius.circular(18)),child:ListTile(onTap:()async{await ref.read(engagementRepositoryProvider).readNotification('${n['id']}');await _load();},leading:Icon(unread?LucideIcons.bellRing:LucideIcons.bell,size:21,color:unread?spikeRed:Colors.black54),title:Text('${n['title']??''}',style:TextStyle(fontWeight:unread?FontWeight.w800:FontWeight.w600)),subtitle:Text('${n['body']??''}',style:const TextStyle(fontSize:11)),trailing:unread?const CircleAvatar(radius:4,backgroundColor:spikeRed):null));})));}}
