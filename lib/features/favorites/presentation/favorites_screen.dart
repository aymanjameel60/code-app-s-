  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(body: SafeArea(child: Column(children: [
      ...
      Expanded(child: state.when(
        ...
        data: (products) {
          if (products.isEmpty) {
            return Column(mainAxisAlignment:MainAxisAlignment.center,children:[Container(width:72,height:72,decoration:const BoxDecoration(color:spikePanel,shape:BoxShape.circle),child:const Icon(LucideIcons.heart,size:30,color:spikeRed)),const SizedBox(height:14),const Text('المفضلة فارغة',style:TextStyle(fontSize:17,fontWeight:FontWeight.w900)),const SizedBox(height:5),const Text('احفظ المنتجات التي تعجبك لتجدها هنا بسهولة',style:TextStyle(fontSize:11,color:spikeMuted)),const SizedBox(height:18),FilledButton(style:FilledButton.styleFrom(backgroundColor:Colors.black),onPressed:()=>context.go('/'),child:const Text('ابدأ التسوق'))]);
          }
          return RefreshIndicator(onRefresh:()async{ref.invalidate(favoritesProvider);await ref.read(favoritesProvider.future);},child:GridView.builder(padding:const EdgeInsets.fromLTRB(17,10,17,24),itemCount:products.length,gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:2,crossAxisSpacing:11,mainAxisSpacing:11,mainAxisExtent:246),itemBuilder:(context,i){
            final p=products[i];
            return SpikeProductCard(product:p,onTap:()=>context.push('/product/${p.id}'),onStore:p.storeId==null?null:()=>context.push('/store/${p.storeId}'),onFavorite:()async{await ref.read(engagementRepositoryProvider).removeWishlist(p.id);ref.invalidate(favoritesProvider);if(context.mounted)showSpikeToast(context,'تمت إزالة المنتج من المفضلة');},onAdd:p.cheapestVariant==null?null:()async{try{await ref.read(cartRepositoryProvider).add(variantId:p.cheapestVariant!.id);if(context.mounted)showSpikeToast(context,'تمت إضافة المنتج إلى السلة');}catch(e){if(context.mounted)showSpikeToast(context,e.toString());}});
          }));
        },
      ))
    ])));
