                  ...
                  children: [
                    ...
                    if (items.isEmpty)
                      const Padding(
                        ...
                        child: Text('لا يوجد عنوان محفوظ بعد. أضف عنوان التوصيل الأول.', textAlign: TextAlign.center, style: TextStyle(fontSize: 11, color: spikeMuted)),
                      ),
...
class _AddressFormScreenState extends ConsumerState<AddressFormScreen> {
  ...
  void _setType(String type) {
    setState(() {
      label = type == 'office' ? 'مكتب' : type == 'other' ? 'أخرى' : 'المنزل';
    });
  }
                      ...
                      Expanded(
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            value: cityId,
                            isExpanded: true,
                            hint: const Text('اختر مدينة التغطية *', style: TextStyle(fontSize: 13, color: spikeMuted)),
                            items: list.map((c) => DropdownMenuItem(value: c.id, child: Text(c.name, style: const TextStyle(fontSize: 13)))).toList(),
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        ...
                        Switch(value: active, onChanged: (v) => setState(() => active = v), activeThumbColor: const Color(0xFFF0AD14), activeTrackColor: const Color(0xFFF0AD14).withValues(alpha: .45)),
                      ]),
                    ),
                  ),
                  const SizedBox(height: 14),
                  Row(children: [
                    Expanded(
                      child: SizedBox(
                        height: 48,
                        child: FilledButton(
                          style: FilledButton.styleFrom(backgroundColor: const Color(0xFFF4B219), foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))),
                          onPressed: busy ? null : save,
                          child: busy ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('حفظ العنوان', style: TextStyle(fontWeight: FontWeight.w700)),
                        ),
                      ),
                    ),
                    ...
                    Expanded(
                      child: SizedBox(
                        height: 48,
                        child: OutlinedButton(onPressed: () => context.pop(), style: OutlinedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), child: const Text('إلغاء')),
                      ),
                    ),
                  ]),
                ],
        ...
        child: Row(children: [
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(fontSize: 13, color: spikeMuted), border: InputBorder.none, enabledBorder: InputBorder.none, focusedBorder: InputBorder.none),
            ),
          ),
        ]),
      );
...
class _AddressType extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: Container(
          height: 83,
          decoration: BoxDecoration(
            color: selected ? const Color(0xFFFFF9E8) : (Theme.of(context).brightness == Brightness.dark ? const Color(0xFF1D1D1D) : Colors.white),
            borderRadius: BorderRadius.circular(16),
