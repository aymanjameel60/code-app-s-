  ...
  void _showEditProfileSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          padding: EdgeInsets.fromLTRB(17, 10, 17, MediaQuery.viewInsetsOf(sheetContext).bottom + 22),
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Center(child: Container(width: 42, height: 4, decoration: BoxDecoration(color: Theme.of(sheetContext).dividerColor, borderRadius: BorderRadius.circular(4)))),
            const SizedBox(height: 14),
            Row(children: [
              const Expanded(child: Text('تعديل الملف الشخصي', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700))),
            ...
            const Text('الاسم', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            TextField(controller: name, decoration: const InputDecoration(hintText: 'الاسم الكامل')),
            const SizedBox(height: 14),
            SizedBox(
              height: 39,
              child: FilledButton(
                ...
                child: const Text('حفظ التعديلات', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
              ),
            ),
          ]),
        ),
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            const Text('تغيير رقم الجوال', textAlign: TextAlign.center, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            const Text('أدخل رقم الجوال الجديد وسنرسل إليه رمز تحقق OTP لتأكيد الرقم.', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: spikeMuted, height: 1.5)),
            const SizedBox(height: 16),
            TextField(controller: phone, keyboardType: TextInputType.phone, textDirection: TextDirection.ltr, decoration: const InputDecoration(hintText: '967 700 000 000')),
            const SizedBox(height: 14),
            SizedBox(height: 39, child: FilledButton(style: FilledButton.styleFrom(backgroundColor: spikeRed), onPressed: busy ? null : _requestPhone, child: const Text('إرسال رمز التحقق'))),
          ]),
        ),
      ),
    );
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            const SizedBox(height: 12),
            const Text('تأكيد رقم الجوال', textAlign: TextAlign.center, style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            Text('أرسلنا رمز تحقق إلى ${phone.text.trim()}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 10, color: spikeMuted)),
            const SizedBox(height: 16),
            TextField(controller: code, maxLength: 6, keyboardType: TextInputType.number, textDirection: TextDirection.ltr, textAlign: TextAlign.center, decoration: const InputDecoration(hintText: '••••••', counterText: '')),
            const SizedBox(height: 14),
            SizedBox(height: 39, child: FilledButton(style: FilledButton.styleFrom(backgroundColor: spikeRed), onPressed: busy ? null : _verifyPhone, child: const Text('تأكيد الرقم'))),
            ...
            children: [
              Container(
                ...
                child: Column(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Theme.of(context).colorScheme.onSurface, width: 1)),
                    child: CircleAvatar(
                      backgroundColor: surface,
                      backgroundImage: avatar.isEmpty ? null : CachedNetworkImageProvider(avatar),
                      child: avatar.isEmpty ? const Icon(LucideIcons.userRound, size: 30) : null,
                    ),
                  ),
                  const SizedBox(height: 12),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: const Text('تعديل الملف الشخصي', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                ),
              ),
              const SizedBox(height: 22),
              const Text('معلومات الحساب', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
              const SizedBox(height: 10),
              Container(
                decoration: BoxDecoration(color: panel, borderRadius: BorderRadius.circular(24)),
                ...
                child: Column(children: [
                  ...
                  _InfoRow(icon: LucideIcons.globe2, label: 'البلد', value: country),
                ]),
              ),
            ],
          );
