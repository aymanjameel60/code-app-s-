...
class HomeData {
  const HomeData({required this.categories, required this.products, required this.stores, required this.banners, required this.collections});
...
class HomeRepository {
  ...
  Future<HomeData> load() async {
    final results = await Future.wait<Map<String, dynamic>>([_api.get('/categories'), _api.get('/products'), _api.get('/stores'), _api.get('/home-sections')]);
    ...
    final banners = (home['banner_items'] as List? ?? const []).whereType<Map>().where((e) => e['enabled'] != false).map((e) => BannerItem.fromJson(Map<String, dynamic>.from(e))).where((e) => e.imageUrl.isNotEmpty).toList();
    ...
    return HomeData(categories: categories, products: products, stores: stores, banners: banners, collections: collections);
  }
}
