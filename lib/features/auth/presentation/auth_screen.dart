...
class _AuthScreenState extends ConsumerState<AuthScreen> {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.all(17), children: [
      ...
      _field(_password, 'ادخل كلمة السر', LucideIcons.lockKeyhole, obscure: true),
      ...
      const Text('OTP غير مفعّل لأن الـ Backend الحالي لا يوفّر endpoint حقيقي له بعد.', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: Colors.black45)),
    ]),
  );

  Widget _field(TextEditingController controller, String hint, IconData icon, {bool obscure = false, TextInputType? keyboard}) => TextField(
    controller: controller,
    obscureText: obscure,
    keyboardType: keyboard,
    decoration: InputDecoration(hintText: hint, filled: true, fillColor: spikeField, suffixIcon: Icon(icon, size: 20), border: const OutlineInputBorder(borderSide: BorderSide.none, borderRadius: BorderRadius.all(Radius.circular(18)))),
  );
}
