  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(body:SafeArea(child:Column(children:[
      ...
      Expanded(child:state.when(loading:()=>const SpikeLoading(),error:(e,_)=>SpikeErrorState(message:e.toString(),onRetry:()=>ref.invalidate(allProductsProvider)),data:(products){
        ...
        return Column(children:[Padding(padding:const EdgeInsets.fromLTRB(17,8,17,3),child:Align(alignment:Alignment.centerRight,child:Text('${list.length} نتيجة',style:const TextStyle(fontSize:11,color:spikeMuted,fontWeight:FontWeight.w600)))),Expanded(child:GridView.builder(padding:const EdgeInsets.fromLTRB(17,5,17,24),itemCount:list.length,gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:2,crossAxisSpacing:11,mainAxisSpacing:11,mainAxisExtent:246),itemBuilder:(context,i){final p=list[i];return SpikeProductCard(product:p,onTap:()=>context.push('/product/${p.id}'),onStore:p.storeId==null?null:()=>context.push('/store/${p.storeId}'),onAdd:p.cheapestVariant==null?null:()async{try{await ref.read(cartRepositoryProvider).add(variantId:p.cheapestVariant!.id);if(context.mounted)showSpikeToast(context,'تمت إضافة المنتج إلى السلة');}catch(e){if(context.mounted)showSpikeToast(context,e.toString());}},onFavorite:()async{try{await ref.read(engagementRepositoryProvider).addWishlist(p.id);if(context.mounted)showSpikeToast(context,'تمت إضافة المنتج إلى المفضلة');}catch(e){if(context.mounted)showSpikeToast(context,e.toString());}});} ))]);
      }))
    ])));
  }
}
