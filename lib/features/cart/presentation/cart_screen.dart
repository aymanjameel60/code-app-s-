            ...
            child: SafeArea(
              ...
              child: SizedBox(
                ...
                child: FilledButton(
                  ...
                  onPressed: () async { final user=await ref.read(currentUserProvider.future); if (!context.mounted) return; if (user == null) { final login=await showDialog<bool>(context: context, builder: (d) => AlertDialog(title: const Text('سجّل الدخول لإكمال الطلب'), content: const Text('يمكنك الاحتفاظ بالمنتجات في السلة، لكن يلزم تسجيل الدخول للمتابعة إلى الدفع.'), actions: [TextButton(onPressed: () => Navigator.pop(d, false), child: const Text('تراجع')), FilledButton(onPressed: () => Navigator.pop(d, true), child: const Text('تسجيل الدخول'))])); if (login == true && context.mounted) context.push('/login'); return; } context.push('/checkout'); },
