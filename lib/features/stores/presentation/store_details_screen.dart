            ...
            children:[
              ...
              for(final option in const [
                ...
                _SortOption(
                  ...
                  onTap:(){setState(()=>_sort=option.$1);Navigator.pop(sheetContext);},
                ),
            ],
          ),
        ),
                                  ...
                                  Expanded(
                                    child:Container(
                                      ...
                                      child:TextField(
                                        ...
                                        decoration:InputDecoration(hintText:'ابحث داخل ${store.name}',prefixIcon:const Icon(Icons.search,size:20),border:InputBorder.none,contentPadding:const EdgeInsets.symmetric(vertical:10)),
                                      ),
                                    ),
                                  ),
                                ],
                              ...
                              child:Column(
                                children:[
                                  ...
                                  Transform.translate(
                                    ...
                                    child:Padding(
                                      ...
                                      child:Row(
                                        ...
                                        children:[
                                          Container(
                                            width:70,height:70,clipBehavior:Clip.antiAlias,
                                            decoration:BoxDecoration(color:Colors.white,shape:BoxShape.circle,border:Border.all(color:const Color(0xFFEEEEEE))),
                                            child:store.logoUrl==null?const Icon(Icons.storefront_outlined,color:Colors.black):CachedNetworkImage(imageUrl:store.logoUrl!,fit:BoxFit.contain,errorWidget:(_,__,___)=>const Icon(Icons.storefront_outlined,color:Colors.black)),
                                          ),
                                          ...
                                          Container(
                                            ...
                                            decoration:BoxDecoration(color:dark?Colors.white10:const Color(0xFFF1F1F1),borderRadius:BorderRadius.circular(18)),
                                            child:Row(children:[const Icon(Icons.star_rounded,size:14,color:Color(0xFFF5B400)),const SizedBox(width:2),Text(reviewsCount>0?rating.toStringAsFixed(1):'—',style:const TextStyle(fontSize:11,fontWeight:FontWeight.w600)),if(reviewsCount>0)Text(' ($reviewsCount)',style:const TextStyle(fontSize:9,color:spikeMuted))]),
                                          ),
                                        ],
                                      ),
                                    ),
                                  ...
                                  Transform.translate(
                                    ...
                                    child:Padding(
                                      padding:const EdgeInsets.symmetric(horizontal:17),
                                      child:Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children:[_stat('${storeProducts.length}','منتجات'),_stat('$reviewsCount','مراجعات'),_stat(reviewsCount>0?rating.toStringAsFixed(1):'—','التقييم')]),
                                    ),
                                  ),
                                ],
                              ),
                                ...
                                children:[
                                  SizedBox(
                                    ...
                                    child:OutlinedButton.icon(
                                      onPressed:()=>setState(()=>_category=_category=='الكل'?(categories.isEmpty?'الكل':categories.first):'الكل'),
                                      icon:const Icon(Icons.tune,size:17),
                                      label:const Text('فلتر',style:TextStyle(fontSize:11)),
                                      style:OutlinedButton.styleFrom(backgroundColor:dark?spikeDarkPanel:spikeField,side:BorderSide.none,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(20))),
                                    ),
                                  ),
                                  ...
                                  SizedBox(
                                    ...
                                    child:OutlinedButton.icon(
                                      ...
                                      label:const Text('ترتيب',style:TextStyle(fontSize:11)),
                                      style:OutlinedButton.styleFrom(backgroundColor:dark?spikeDarkPanel:spikeField,side:BorderSide.none,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(20))),
                                    ),
                                  ),
                                ],
                              ...
                              Padding(
                                ...
                                child:SizedBox(
                                  ...
                                  child:ListView(
                                    ...
                                    children:[
                                      ChoiceChip(label:const Text('الكل',style:TextStyle(fontSize:10)),selected:_category=='الكل',onSelected:(_)=>setState(()=>_category='الكل'),showCheckmark:false),
                                      for(final category in categories)...[
                                        const SizedBox(width:7),
                                        ChoiceChip(label:Text(category,style:const TextStyle(fontSize:10)),selected:_category==category,onSelected:(_)=>setState(()=>_category=category),showCheckmark:false),
                                      ],
                                    ],
                                  ),
                                ),
                            ...
                            if(reviews.isNotEmpty)
                              Padding(
                                ...
                                child:Column(
                                  crossAxisAlignment:CrossAxisAlignment.stretch,
                                  children:[
                                    Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[const Text('تقييمات العملاء',style:TextStyle(fontSize:16,fontWeight:FontWeight.w700)),Text('${reviews.length} تعليق',style:const TextStyle(fontSize:10,color:spikeMuted))]),
...
class _SortOption extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>InkWell(
    ...
    child:SizedBox(
      height:48,
      child:Row(children:[Expanded(child:Text(label,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w700))),Container(width:19,height:19,decoration:BoxDecoration(shape:BoxShape.circle,border:Border.all(color:Theme.of(context).colorScheme.onSurface,width:1.5)),child:selected?Center(child:Container(width:11,height:11,decoration:const BoxDecoration(color:Colors.black,shape:BoxShape.circle))):null)]),
    ),
  );
}
